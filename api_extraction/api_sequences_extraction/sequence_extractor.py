...
import numpy as np
import time
from producer import Stage3Producer
from producer import Stage3TimeShiftProducer
import json
import sys
from producer import Stage2Producer
from producer import Stage2TimeShiftProducer
# to store and update of each function in the run
# to store the call stack (used to identify recursion)
#function_call_stack = []
# to store the api sequence

# We have to add coverage
class ApisExtraction:
    def __init__(self, sample_hash, sample_data, total_api_sequences, api_sequence_budget):
        '''
    ...
    '''
        the following function returns functions that have less than 100% API coverage
    '''
    ...
    '''
        the following function replaces the important block(the ones containing calls) with our cleaned ones
    '''
    ...
    def graph_parser(self,function_addr, block_addr, sequence, start_time, count_recursion, call_list_dictionary, api_function_and_blocks):
        #print("Called : {}---{}".format(function_addr, block_addr))
        ...
        while 1:
            ...
            if sum(relative_weights) > 0:
                ...
                #non_zero = [node_id for node_id in range(len(relative_weights)) if relative_weights[node_id]!= 0]
                '''
                ...
                #totally random
                #non_zero = [node_id for node_id in range(len(relative_weights)) if relative_weights[node_id]!= 0]
                #next_block_id = random.choices(non_zero, k=1)[0]
                #probabilistic               
                ...
                #print("next block to be visited : {}----{}".format(function_addr, block_addr))
                continue
                ...
                if len(self.function_stack) > 1:
                    ...
                    #print("returning to prev function from stack {}".format(function_addr))
                    break
                else:
                    #2.1.2.2
                    #we are in the root function, we now check the xrefs of the root function to continue the execution
                    #print("Current function is the root function, thus checking xrefs")
                    if function_addr not in self.xrefs:
                        ...
                        # assert False
                        if flag == 0:
                            #print("something wrong with xrefs")
                            break
                        ...
                        if function_addr_new in self.wt_matrix and function_addr_new in self.index_of_block and \
                            ...
                            break
        #print("before reducing function stack the value is {}".format( self.function_stack))
    ...
    '''
        This function initializes the function, and picks a random block to start exploration
    '''
    def sequence_extractor(self, function_addr, start_blocks_available=[]):
        ...
        #a more neat approach is possible. But atm I am lazy
    ...
    def api_extractor(self):
        ...
        while itr < self.iterations:
            ...
            if len(start_funcs_available) < 3:
                ...
                function_addr = start_funcs_available[random_func_index]
                block_addr_available = start_blocks_available[function_addr]
                #start_funcs_available.remove(function_addr)
