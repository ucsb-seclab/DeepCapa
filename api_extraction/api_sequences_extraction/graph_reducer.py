...
class GraphReduction:
    #def __init__(self, iterations, num_of_apis, extracted_api_path, extracted_graph_path, extracted_xref_path, hash, output_repo_path):
    def __init__(self, sample_hash, sample_data, total_api_sequences=[], api_sequences_budget=0):
        '''
        ...
        self.num_of_apis = api_sequences_budget
    ...
    def fetch_incoming_and_outgoing_edge_dict(self, wt_matrix):
        ...
        :return: new incoming_edge_dict for the updated graph
        '''
        incoming_edge_dict = {}
        outgoing_edge_dict = {}
        end_edges = []
    ...
    def initializer(self, function_addr, function_block_api_dictionary, G):
        ...
        '''
        important_nodes dictionary contains key and cleaned value of blocks that contain function/api calls
        '''
        ...
        # 3
        #extracting the adjacency matrix of current graph
    ...
    '''
        Initialzing the CFG of function, calling the reduce graph function and storing the reduced graph
    '''
    #def initialize_and_reduce_all_call_funcs(self, function_addr_containing_calls, combined_dict, function_block_api_dictionary):
    def initialize_and_reduce_all_call_funcs(self, function_addr_containing_calls, graph_dictionary, function_block_api_dictionary):
        #print("total functions: {}".format(len(function_block_api_dictionary)))
        #parsing through each function
        bef_time = time.time()
        for func_addr in function_addr_containing_calls:
            # func_addr = "0x4011f0"
            # print(func_addr)
            #G = nx.node_link_graph(combined_dict[func_addr]['extracted_graph'])
    ...
    def graph_reducer(self):
        ...
        if ret == 1:
            ...
            #message = {"method": "push", "data": new_combined_dict}
            stage2_dict = new_combined_dict
            #sequences_to_extract = 0
            sequences_to_extract = 350
            # if len(func_addr_containting_apis) > 450:
            #     sequences_to_extract = len(func_addr_containting_apis) - 200
            # else:
            #     sequences_to_extract = 250
            obj = ApisExtraction(stage2_dict['hash'], stage2_dict, sequences_to_extract, 20)
