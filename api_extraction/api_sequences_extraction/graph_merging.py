...
def caller_check(function_addr, block_addr, xrefs):
    ...
    return flag
'''
    This function merges the new caller addresses to existing caller addresses
'''
...
# Here call_addr_super is call_addr for the block
# def merge_call_addr_block_ordering(superset_id, subset_id, superset_block_call, subset_block_call, \
#     call_addr_dictionary, call_addr_super, call_addr_sub, function_addr, block_addr, sample_data):
...
def merging_at_block_level_for_subset_graph(current_saved_id, function_addr, sample_data, xref_dictionary, call_addr_dictionary, func_block_api_dictionary, graph_dictionary, is_duplicate_flag):
    ...
    for call_block_addr in call_dict_2:
        #When there exist a block in subset that is not present in superset.. Ideally this should never happen
        #We also have to add this to the graph no?
        if call_block_addr not in call_dict_1:
            #print("block addr not in call list")
        ...
        if call_dict_2[call_block_addr] == call_dict_1[call_block_addr]:
            ...
            #call_dict_1[call_block_addr] = call_list
    for block in new_call_list_dict:
        temp = list()
        temp_addr_list = list()
        if len(new_call_list_dict[block]) != len(call_addr_dictionary[function_addr][block]):
            print("\nError in merging! Block not present in superset: Block addr: {}, needs manual inspection".format(call_block_addr))
            continue
